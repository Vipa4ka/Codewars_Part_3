function getCard() {
    function TTT(name, length, min, max) {
        let arr = [];
        while (arr.length < length) {
            let randomNumber = Math.floor(Math.random() * (max - min + 1)) + min;
            if (arr.indexOf(name + randomNumber) === -1) { arr.push(name + randomNumber) };
        }
        return (arr)
    }
    return([...TTT("B", 5, 1, 15), ...TTT("I", 5, 16, 30), ...TTT("N", 4, 31, 45), ...TTT("G", 5, 46, 60), ...TTT("O", 5, 61, 75)]);
}
console.log(getCard());
;
